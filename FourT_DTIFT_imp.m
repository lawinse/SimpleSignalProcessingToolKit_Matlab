function varargout = FourT_DTIFT_imp(varargin)
% FOURT_DTIFT_IMP MATLAB code for FourT_DTIFT_imp.fig
%      FOURT_DTIFT_IMP, by itself, creates a new FOURT_DTIFT_IMP or raises the existing
%      singleton*.
%
%      H = FOURT_DTIFT_IMP returns the handle to a new FOURT_DTIFT_IMP or the handle to
%      the existing singleton*.
%
%      FOURT_DTIFT_IMP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FOURT_DTIFT_IMP.M with the given input arguments.
%
%      FOURT_DTIFT_IMP('Property','Value',...) creates a new FOURT_DTIFT_IMP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FourT_DTIFT_imp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FourT_DTIFT_imp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FourT_DTIFT_imp

% Last Modified by GUIDE v2.5 29-Jun-2015 21:19:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FourT_DTIFT_imp_OpeningFcn, ...
                   'gui_OutputFcn',  @FourT_DTIFT_imp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FourT_DTIFT_imp is made visible.
function FourT_DTIFT_imp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FourT_DTIFT_imp (see VARARGIN)

% Choose default command line output for FourT_DTIFT_imp
...
% UIWAIT makes FourT_DTIFT_imp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FourT_DTIFT_imp_OutputFcn(hObject, eventdata, handles) 
...
try
[y1,n] = dt_imp_plot(handles.metricdata.x0,handles.metricdata.xl,handles.metricdata.xr,handles.metricdata.c0,0);
[Y,nn] = dt_fft(y1,n);
dt_ifft(n,Y);
catch 
