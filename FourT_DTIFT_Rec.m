function varargout = FourT_DTIFT_Rec(varargin)
% FOURT_DTIFT_REC MATLAB code for FourT_DTIFT_Rec.fig
%      FOURT_DTIFT_REC, by itself, creates a new FOURT_DTIFT_REC or raises the existing
%      singleton*.
%
%      H = FOURT_DTIFT_REC returns the handle to a new FOURT_DTIFT_REC or the handle to
%      the existing singleton*.
%
%      FOURT_DTIFT_REC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FOURT_DTIFT_REC.M with the given input arguments.
%
%      FOURT_DTIFT_REC('Property','Value',...) creates a new FOURT_DTIFT_REC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FourT_DTIFT_Rec_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FourT_DTIFT_Rec_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FourT_DTIFT_Rec

% Last Modified by GUIDE v2.5 30-Jun-2015 20:14:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FourT_DTIFT_Rec_OpeningFcn, ...
                   'gui_OutputFcn',  @FourT_DTIFT_Rec_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FourT_DTIFT_Rec is made visible.
function FourT_DTIFT_Rec_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FourT_DTIFT_Rec (see VARARGIN)

% Choose default command line output for FourT_DTIFT_Rec
...
% UIWAIT makes FourT_DTIFT_Rec wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FourT_DTIFT_Rec_OutputFcn(hObject, eventdata, handles) 
...
try
[y1,n] = dt_Rec_plot(handles.metricdata.x0,handles.metricdata.c1,handles.metricdata.xl,handles.metricdata.xr,handles.metricdata.c0,0);
[Y,nn] = dt_fft(y1,n);
dt_ifft(n,Y);
catch 
