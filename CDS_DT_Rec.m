function varargout = CDS_DT_Rec(varargin)
% CDS_DT_REC MATLAB code for CDS_DT_Rec.fig
%      CDS_DT_REC, by itself, creates a new CDS_DT_REC or raises the existing
%      singleton*.
%
%      H = CDS_DT_REC returns the handle to a new CDS_DT_REC or the handle to
%      the existing singleton*.
%
%      CDS_DT_REC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CDS_DT_REC.M with the given input arguments.
%
%      CDS_DT_REC('Property','Value',...) creates a new CDS_DT_REC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before CDS_DT_Rec_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CDS_DT_Rec_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CDS_DT_Rec

% Last Modified by GUIDE v2.5 29-Jun-2015 13:55:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @CDS_DT_Rec_OpeningFcn, ...
                   'gui_OutputFcn',  @CDS_DT_Rec_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CDS_DT_Rec is made visible.
function CDS_DT_Rec_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to CDS_DT_Rec (see VARARGIN)

% Choose default command line output for CDS_DT_Rec
...
% UIWAIT makes CDS_DT_Rec wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CDS_DT_Rec_OutputFcn(hObject, eventdata, handles) 
...
try
[y1] = dt_Rec_plot(handles.metricdata.x0,handles.metricdata.c1,handles.metricdata.xl,handles.metricdata.xr,handles.metricdata.c0);
catch 
