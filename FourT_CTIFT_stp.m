function varargout = FourT_CTIFT_stp(varargin)
% FOURT_CTIFT_STP MATLAB code for FourT_CTIFT_stp.fig
%      FOURT_CTIFT_STP, by itself, creates a new FOURT_CTIFT_STP or raises the existing
%      singleton*.
%
%      H = FOURT_CTIFT_STP returns the handle to a new FOURT_CTIFT_STP or the handle to
%      the existing singleton*.
%
%      FOURT_CTIFT_STP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FOURT_CTIFT_STP.M with the given input arguments.
%
%      FOURT_CTIFT_STP('Property','Value',...) creates a new FOURT_CTIFT_STP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FourT_CTIFT_stp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FourT_CTIFT_stp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FourT_CTIFT_stp

% Last Modified by GUIDE v2.5 29-Jun-2015 21:21:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FourT_CTIFT_stp_OpeningFcn, ...
                   'gui_OutputFcn',  @FourT_CTIFT_stp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FourT_CTIFT_stp is made visible.
function FourT_CTIFT_stp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FourT_CTIFT_stp (see VARARGIN)

% Choose default command line output for FourT_CTIFT_stp
...
% UIWAIT makes FourT_CTIFT_stp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FourT_CTIFT_stp_OutputFcn(hObject, eventdata, handles) 
...
try
[y1] = ct_step_plot(handles.metricdata.x0,handles.metricdata.xl,handles.metricdata.xr,handles.metricdata.c0,handles.metricdata.c1,0);
Y = ct_fft(y1);
ct_ifft(Y);
catch 
