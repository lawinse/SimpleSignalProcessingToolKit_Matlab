function varargout = Sampl_CT_stp(varargin)
% SAMPL_CT_STP MATLAB code for Sampl_CT_stp.fig
%      SAMPL_CT_STP, by itself, creates a new SAMPL_CT_STP or raises the existing
%      singleton*.
%
%      H = SAMPL_CT_STP returns the handle to a new SAMPL_CT_STP or the handle to
%      the existing singleton*.
%
%      SAMPL_CT_STP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SAMPL_CT_STP.M with the given input arguments.
%
%      SAMPL_CT_STP('Property','Value',...) creates a new SAMPL_CT_STP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Sampl_CT_stp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Sampl_CT_stp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Sampl_CT_stp

% Last Modified by GUIDE v2.5 27-Jun-2015 22:19:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Sampl_CT_stp_OpeningFcn, ...
                   'gui_OutputFcn',  @Sampl_CT_stp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Sampl_CT_stp is made visible.
function Sampl_CT_stp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Sampl_CT_stp (see VARARGIN)

% Choose default command line output for Sampl_CT_stp
...
% UIWAIT makes Sampl_CT_stp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Sampl_CT_stp_OutputFcn(hObject, eventdata, handles) 
...
xco = handles.metricdata.xco;
try
    [y,x,p] = ct_step_plot_private(x0,xl,xr,co,xco,0);
    [ya,xa Ya,Xa] = LPF(y,x,handles.metricdata.lpf_n,handles.metricdata.lpf_l,handles.metricdata.lpf_r,handles.metricdata.lpf_co);
catch 
...
function inputXc_Callback(hObject, eventdata, handles)
% hObject    handle to inputXc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
xco = eval(get(hObject, 'String'));
if isnan(xco)
    set(hObject, 'String', 0);
    errordlg('Input must be a number','Error');
end
handles.metricdata.xco = xco;
guidata(hObject,handles);
% Hints: get(hObject,'String') returns contents of inputXc as text
%        eval(get(hObject,'String')) returns contents of inputXc as a double


% --- Executes during object creation, after setting all properties.
function inputXc_CreateFcn(hObject, eventdata, handles)
% hObject    handle to inputXc (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
set(handles.inputXc, 'String', handles.metricdata.xco);
