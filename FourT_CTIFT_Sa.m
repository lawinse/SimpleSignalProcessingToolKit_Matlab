function varargout = FourT_CTIFT_Sa(varargin)
% FOURT_CTIFT_SA MATLAB code for FourT_CTIFT_Sa.fig
%      FOURT_CTIFT_SA, by itself, creates a new FOURT_CTIFT_SA or raises the existing
%      singleton*.
%
%      H = FOURT_CTIFT_SA returns the handle to a new FOURT_CTIFT_SA or the handle to
%      the existing singleton*.
%
%      FOURT_CTIFT_SA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FOURT_CTIFT_SA.M with the given input arguments.
%
%      FOURT_CTIFT_SA('Property','Value',...) creates a new FOURT_CTIFT_SA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FourT_CTIFT_Sa_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to FourT_CTIFT_Sa_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FourT_CTIFT_Sa

% Last Modified by GUIDE v2.5 29-Jun-2015 20:41:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FourT_CTIFT_Sa_OpeningFcn, ...
                   'gui_OutputFcn',  @FourT_CTIFT_Sa_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FourT_CTIFT_Sa is made visible.
function FourT_CTIFT_Sa_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FourT_CTIFT_Sa (see VARARGIN)

% Choose default command line output for FourT_CTIFT_Sa
...
% UIWAIT makes FourT_CTIFT_Sa wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FourT_CTIFT_Sa_OutputFcn(hObject, eventdata, handles) 
...
[y1] = ct_Sa_plot(handles.metricdata.x0,handles.metricdata.xl,handles.metricdata.xr,handles.metricdata.c0,0);
Y = ct_fft(y1);
ct_ifft(Y);
catch 
